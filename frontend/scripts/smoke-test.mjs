const baseUrl = process.env.SMOKE_BASE_URL || 'http://127.0.0.1:5690';
...
const adminCookie = await login('admin@construction-equipment-rental.local', 'admin123');
const managerCookie = await login('manager@construction-equipment-rental.local', 'manager123');
const analystCookie = await login('analyst@construction-equipment-rental.local', 'analyst123');
